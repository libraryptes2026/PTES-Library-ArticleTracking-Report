...
# --- 🔄 TRANSACTIONAL SAFE RETRY EXECUTION ENGINE ---
def execute_transaction_with_retry(func, *args, **kwargs):
    """Executes a database operations function with a cooling window if rate limits are hit."""
    max_retries = 3
    delay = 2
    for attempt in range(max_retries):
        try:
            ...
            if "429" in str(e.code) or attempt == max_retries - 1:
            ...
            if not class_registry:
                ...
                if st.button("🔥 Submit Tally Data Logs", use_container_width=True):
                    try:
                        ...
                        # Apply safe transaction retry logic to absorb 429 errors cleanly
                        execute_transaction_with_retry(challenge_db.append_row, new_tally_row, value_input_option="USER_ENTERED")
                        
                        # Invalidate specific cache functions instead of completely clearing the entire application context
                        fetch_registry_rows.clear(target_spreadsheet)
                        fetch_leaderboard_range.clear(target_spreadsheet)
                        ...
                        st.error(f"Failed to record data due to API limits: {e}. Please wait a few seconds and try again.")
                                ...
                                for date_idx, date_str in enumerate(date_list):
                                    ...
                                    with col_btn:
                                        ...
                                        if st.button("❌ Remove Date", key=btn_key, use_container_width=True):
                                            ...
                                            if len(date_list) == 0:
                                                execute_transaction_with_retry(log_sheet.delete_rows, actual_sheet_row)
                                                st.toast("Empty month entry dropped completely!")
                                            else:
                                                new_dates_str = ", ".join(date_list)
                                                new_count = len(date_list)
                                                
                                                execute_transaction_with_retry(log_sheet.update_cell, actual_sheet_row, 5, new_count)
                                                execute_transaction_with_retry(log_sheet.update_cell, actual_sheet_row, 6, new_dates_str)
